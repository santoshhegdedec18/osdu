...
uri = "http://ontology.osdu.com/osdu_taxonomy.owl" 
...
class_well = types.new_class("WELL",(Thing,),kwds=None)
...
def create_well_class(class_well):
...
def create_wellbore_class(class_wellbore):
...
def create_well_log_class(class_well_log):
...
def handle_single_dictionary_element(key, value, ontoclass):
    if key not in props_tobe_ignored:
        annot_prop=types.new_class(key.replace('ID',''),(AnnotationProperty,),kwds=None)
...
def handle_list_object(key, lst, ontoclass):
    for item in lst:
        if isinstance(item,list):
            class_lst = types.new_class(key.upper().replace('ID',''),(ontoclass,),kwds=None)
            if key != 'description':
...
def handle_group_headers(key, ontoclass):
    if key not in headers_tobe_ignored:
        if key != 'description':
            class_obj = types.new_class(key.upper().replace('ID',''),(ontoclass,),kwds=None)
            annot_prop=types.new_class("label",(AnnotationProperty,),kwds=None)
...
create_well_class(class_well) 
create_wellbore_class(class_wellbore)
create_well_log_class(class_well_log)

osdu_onto.save('D:/OSDU/Ontology/osdu_taxonomy.owl')
