...
def handle_dictionary_object(dic, pelement):
      ...
      for key, value in dic.items():
          ...
          if key != "WorkProduct" or key != "WorkProductComponents":
              
                if key == "ResourceID":
                    parentelement = value
                    documentid = value
                if not type(value) == bool and isinstance(value, dict):
                    pe = handle_group_headers(key, parentelement,documentid)
                    graph_orchestrator(value, pe)
                            
                elif not type(value) == bool and isinstance(value, list):
                    pe = handle_group_headers(key, parentelement,documentid)
                    handle_list_object(key, value, pe, documentid)
                    
                else:
                    parentelement = handle_single_dictionary_element(key, value, parentelement, documentid)
     
          else:
                documentid = dic.get("WellboreID")
                print("inside well product___________%", documentid)
...
def handle_single_dictionary_element(key, value, parentelement, documentid):
    if  key not in structuaral_elements:
        nodequery = '''MERGE ( '''+str(key) +''':Welllog {name:"'''+str(key)+'''", value:"'''+str(value)+'''", id:"'''+documentid+'''"})
        WITH '''+str(key)+''' MATCH (a),(b) where a.value="'''+str(parentelement)+'''" and a.id="'''+documentid+'''" and b.value = "'''+str(value)+'''"  and b.id="'''+documentid+'''"
...
def handle_list_object(key, lst, parentelement, documentid):
    for item in lst:
        if isinstance(item,list):
           ...
           graph_orchestrator(item, parentelement)
            ...
            handle_single_list_item(item, parentelement, documentid)
def handle_single_list_item(item, parentelement, documentid):
        print("Its a list..........%", item)
        nodequery = '''MERGE ( wplog:Welllog {name:"'''+str(item)+'''", value:"'''+str(item)+'''", id:"'''+documentid+'''"})
        WITH wplog MATCH (a),(b) where a.value="'''+str(parentelement)+'''" and a.id="'''+documentid+'''" and b.value = "'''+str(item)+'''"  and b.id="'''+documentid+'''"
        WITH a,b MERGE(b)<-[:has_'''+str(item.replace("-","_"))+''']-(a)'''
        graph_creator(nodequery)
        
def handle_group_headers(key, parentelement, documentid):
    pe = parentelement
    if key not in headers_tobe_ignored:
        nodequery = '''MERGE ( '''+str(key) +''':Welllog {name:"'''+str(key)+'''", value:"'''+str(key)+'''" , id:"'''+documentid+'''"})
        WITH '''+str(key)+''' MATCH (a),(b) where a.value="'''+str(parentelement)+'''" and a.id="'''+documentid+'''" and b.value = "'''+str(key)+'''" and b.id = "'''+documentid+'''"
        WITH a,b MERGE(b)<-[:has_'''+str(key)+''']-(a)'''
        pe = str(key)
        graph_creator(nodequery)
    return pe
